...
//Student note: The LED does not flicker because of the "dead zone" implementation. The dead zone
//              effectively changes a single number into a range by having two "checkpoints", one for
//              turning the LED on and one for turning it off. These two checkpoints are separated by
//              a large enough gap that the natural fluctuations that happen when an analogue signal is
//              moving primarily up or down will not be able to trigger both checkpoints, only one.
//              Since the signal will only be able to trigger one checkpoint with its natural fluctuations,
//              the LED does not flicker as it would if there were not a dead zone.


// section 2/3 ****
//Temperature Controlling LED2

        if(temp < temproom * 1.02 && temp > temproom * 1.00)
          {}
        else
          {
            if(temp >= temproom * 1.02) {P1OUT |=  BIT5; __delay_cycles(200); }     //led on if temp hi
            if(temp <= temproom * 1.00) {P1OUT &= ~BIT5; __delay_cycles(200); }     //led off if temp lo
          }
...
//Student note: I changed it to work for 1.02 and 1.00, as specified in the lab manual,
//              rather than 1.03 and 1.01
...
//The following code uses a dead zone of no action between 0.7-0.9 of the value touch
        ...
        if(touch > touchroom * 0.7 && touch < touchroom * 0.9)
          {}
            ...
            //while touching && flag not set
            if ( (touch <= touchroom * 0.7) && (flag == 0) ) {
                P2OUT ^= BIT0;                      //toggle flag
                flag = 1;                           //set flag so this only occurs on first touch
            }
            //while not touching
            else if (touch >= touchroom * 0.9) {
                flag = 0;                           //unset flag again
            }
        }
